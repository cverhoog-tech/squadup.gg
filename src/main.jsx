...
function VoicePage(){
 ...
 const discordUrl=import.meta.env.VITE_DISCORD_CHANNEL_URL || 'https://discord.com/channels/@me';
 const openDiscord=()=>window.open(discordUrl,'_blank');
   ...
   <main className="voiceGamePhone">
    <section className="gameVoiceHero" style={{backgroundImage:`linear-gradient(180deg,rgba(5,7,12,.18),rgba(5,7,12,.92)),linear-gradient(90deg,${theme.glow},transparent 70%),url(${hostGame.hero || hostGame.art})`}}>
      <div className="gameVoiceTop">
       <button><ChevronLeft size={22}/></button>
       <button><span></span><span></span><span></span></button>
      </div>
      <p className="voiceKicker">CURRENT GAME</p>
      <h1>{hostGame.title}</h1>
      <div className="hostLine"><img src={users[0].avatar}/><div><small>HOST</small><b>Shane</b></div><em>● In Voice</em><button onClick={openDiscord}><MessageCircle size={21}/></button></div>
    ...
    <section className="discordChannelCard">
      ...
      <div className="channelActions">
       ...
       <button onClick={openDiscord}><Share2 size={20}/></button>
    ...
    <footer className="voiceGameActions">
      ...
      <button onClick={openDiscord}>↗<span>Discord</span></button>
    </footer>
   </main>
   ...
   <aside className="voiceGameSide">
    ...
    <section className="gameSwitcherPanel">
     ...
     <div className="gameSwitchGrid">
      {games.slice(0,7).map(g=><button key={g.id} className={hostGame.id===g.id?'active':''} onClick={()=>setHostGame(g)} style={{backgroundImage:`linear-gradient(180deg,transparent,rgba(0,0,0,.8)),url(${g.art})`}}>
       <span>{g.title}</span>
      </button>)}
     </div>
    </section>
