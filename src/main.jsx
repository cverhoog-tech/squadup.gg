...
const squadScenarios = [
 {title:'Wednesday raid night', meta:'20:00 · planning', cta:'Start vote', icon:CalendarDays},
 {title:'Nobody can decide', meta:'4 friends online', cta:'Roll game', icon:Dice5},
 {title:'HELLDIVERS squad forming', meta:'3/4 ready · Discord synced', cta:'Join', icon:Shield}
];
const sessionMessages = [
 {name:'Leah', text:'Woensdag om 20:00 lukt. Wat spelen we?', avatar:users[1].avatar},
 {name:'Tom', text:'Ik stem voor Deep Rock of Helldivers.', avatar:users[2].avatar},
 {name:'Maya', text:'Laat de dice maar beslissen als we blijven twijfelen.', avatar:users[3].avatar}
];
const decisionGames = ['helldivers','deep','lethal','valheim'].map(id=>games.find(g=>g.id===id));
...
function App(){
 ...
 const [toast,setToast]=useState('SquadUp v0.7 strategy update ready');
...
function MobileHeader(){return <header className="mobileHeader mobileOnly"><div className="logo"><Zap size={23}/><b>SQUADUP</b><small>v0.7</small></div><div><Search size={18}/><Bell size={18}/><img src={users[0].avatar}/></div></header>}
...
function HomePage({setPage,setSelected,setToast}){
 return <Page><MobileHeader/>
  <section className="desktopHero squadPulseHero">
    <div className="heroBg" style={{backgroundImage:`linear-gradient(90deg,#080a13 8%,rgba(8,10,19,.35)),url(${games[1].hero})`}}>
     <p className="accent">SQUAD PULSE</p><h1>What are we playing tonight?</h1><h2>3 friends ready · 1 decision missing</h2><p>Plan a session, vote with your squad, roll the dice when nobody agrees, then push the result to Discord.</p>
     <button className="primary" onClick={()=>setPage('parties')}>Open Squad Moment</button><button onClick={()=>setToast('Discord poll prepared')}>Send Discord Poll</button>
    </div>
  </section>
  <section className="mobileHomeHero mobileOnly">
   <div className="welcome"><img src={users[0].avatar}/><div><h1>Welcome back, Shane</h1><p>3 friends are almost ready.</p></div></div>
   <SquadCard setPage={setPage}/>
  </section>
  <div className="homeGrid">
   <main>
    <SquadPulsePanel setPage={setPage} setToast={setToast}/>
    <SectionTitle title="Squad Picks for Tonight"/><HorizontalRow items={[games[1],games[11],games[12],games[13],games[8],games[7]]} setSelected={setSelected}/>
    <SquadDock setPage={setPage}/>
    <SectionTitle title="Short Session Winners"/><CardGrid items={[games[12],games[11],games[4],games[8]]} setSelected={setSelected}/>
    <Panel title="Recent Activity"><ActivityList/></Panel>
   </main>
   <aside className="rightPanel desktopOnly"><Panel title="Tonight's Momentum"><SquadPulseMini setPage={setPage}/></Panel><Panel title="Online Friends">{users.slice(1,6).map(u=><UserLine key={u.name} user={u}/>)}</Panel></aside>
  </div>
 </Page>
}

function SquadPulsePanel({setPage,setToast}){
 return <section className="squadPulsePanel">
  <div className="pulseHeader"><div><p className="accent">LIVE COORDINATION</p><h2>Squad Pulse</h2><small>Turn indecision into a game moment.</small></div><button onClick={()=>setPage('parties')}>Open Parties <ChevronRight size={16}/></button></div>
  <div className="pulseScenarioGrid">{squadScenarios.map(({title,meta,cta,icon:Icon},i)=><button key={title} onClick={()=>i===0?setToast('Raid night poll queued for Discord'):setPage('parties')}><Icon size={22}/><b>{title}</b><small>{meta}</small><em>{cta}</em></button>)}</div>
  <div className="pulseVotePreview"><span>Next decision</span><b>Helldivers 2 vs Deep Rock Galactic</b><div><i style={{width:'62%'}}></i></div><small>Leah and Maya voted · Tom is still deciding</small></div>
 </section>
}
function SquadPulseMini({setPage}){return <div className="squadPulseMini"><b>Wednesday Raid Night</b><p>20:00 · 4 invited · 2 ready</p><button className="primary" onClick={()=>setPage('parties')}>Resolve Game Choice</button></div>}
...
function ImmersiveExplore({games,wishlist,toggleWish,setSelected}){
 ...
 const endlessBlocks=[
  {type:'double', a:'Perfect for Your Squad', b:'Friends Can Play Tonight', start:0},
  {type:'split', title:'Squad Decision Picks', mode:'spotlight-left', start:4},
  {type:'compact', title:'Fresh Squad Ideas', start:5},
  {type:'split', title:'Plan Ahead', mode:'list-left', start:7},
  {type:'double', a:'Zero-Frictie Picks', b:'New for Game Night', start:9},
  {type:'split', title:'Co-op Deals', mode:'spotlight-left', start:2},
  {type:'compact', title:'Friday Squad Chaos', start:3},
  {type:'split', title:'Your Squad Wishlist', mode:'list-left', start:6},
  {type:'double', a:'Co-op Squad Decision Picks', b:'Survival Discounts', start:1},
  {type:'split', title:'Group Buy Candidates', mode:'spotlight-left', start:8},
  ...
  {type:'split', title:'Friday Squad Chaos', mode:'list-left', start:15},
  {type:'compact', title:'Short Session Winners', start:16},
 ...
 return <div className="exploreExactV53">
   <section className="exactHero v53Hero" style={{backgroundImage:`linear-gradient(90deg,#070912 7%,rgba(7,9,18,.35) 60%,#070912 100%),url(${hero.hero})`}}>
    ...
    <div className="exactHeroCopy">
      <p className="accent">Squad Recommendation</p>
      <h1>{hero.title}</h1>
      <h2>{hero.sub}</h2>
      <p>{hero.desc}</p>
      <div className="squadFitStrip"><span>3 friends own</span><span>Great 45-90 min session</span><span>Vote-ready</span></div>
...
function PartiesPage({partyMembers,setPartyMembers,setChatOpen,setToast}){
 ...
 const [members,setMembers]=useState(partyMembers.map((m,i)=>({...m,state:['Host','Speaking','Ready','In Game','Away','In Voice'][i]||'Online'})));
 const [votes,setVotes]=useState({helldivers:3,deep:2,lethal:1,valheim:1});
 const [rolled,setRolled]=useState(null);
 const [chat,setChat]=useState(sessionMessages);
 const [chatInput,setChatInput]=useState('');
 const [activities,setActivities]=useState([
  {text:'Maya voted for Deep Rock Galactic', time:'Just now', icon:Dice5, avatar:users[3].avatar, type:'join'},
  {text:`Shane changed the game to ${activeGame.title}`, time:'2m ago', icon:Gamepad2, avatar:users[0].avatar},
  {text:'Leah is ready for Wednesday 20:00', time:'2m ago', icon:CheckCircle2, avatar:users[1].avatar, type:'speaking'},
 ...
 const vote=(g)=>{setVotes(v=>({...v,[g.id]:(v[g.id]||0)+1}));setActiveGame(g);log(`Vote added for ${g.title}`,Dice5,users[0].avatar,'discord')};
 const rollGame=()=>{const pick=decisionGames[Math.floor(Math.random()*decisionGames.length)];setRolled(pick);setActiveGame(pick);log(`Dice roll picked ${pick.title}`,Dice5,users[0].avatar,'discord');setToast(`Dice picked ${pick.title}`)};
 const sendChat=()=>{const clean=chatInput.trim(); if(!clean) return; setChat(c=>[{name:'Shane',text:clean,avatar:users[0].avatar},...c].slice(0,5)); setChatInput(''); log('Squad chat message sent',MessageCircle,users[0].avatar)};
  ...
  <div className="v6126Layout">
   <main className="v6126Main">
    <section className="v6126Hero" style={{backgroundImage:`linear-gradient(180deg,rgba(5,7,12,.08),rgba(5,7,12,.92)),url(${activeGame.hero||activeGame.art})`}}>
     ...
     <p>Squad Moment</p><h1>{activeGame.title}</h1>
     <div className="v6126Host"><img src={users[0].avatar}/><span><small>HOST</small><b>Shane <Crown size={14}/></b></span><em>● Planning + Voice</em></div>
     <div className="v6126Pills"><span>● {members.length} invited</span><span>🎲 Vote active</span><span>Discord Synced</span></div>
     <div className="v6126VoiceCenter"><button onClick={()=>discordToast('Wednesday 20:00 poll sent')}><CalendarDays size={22}/><small>Plan</small></button><i><Dice5 size={48}/></i><button onClick={rollGame}><Dice5 size={22}/><small>Roll</small></button></div>
     <div className="v6126MemberRail">{members.map((m,i)=><PartyAvatar key={m.name} member={m} index={i}/>)}</div>
    </section>
    <SquadDecisionPanel votes={votes} vote={vote} rollGame={rollGame} rolled={rolled}/>
    <section className="v6126SquadActions"><h2>Squad Actions <small>(via Discord)</small></h2><div>
      ...
      <DiscordActionCard Icon={CalendarDays} title="Plan" sub="Wednesday 20:00" accent="#f97316" onClick={()=>discordToast('Raid night invite sent')}/>
      ...
      <DiscordActionCard Icon={MessageCircle} title="Squad Chat" sub="Session only" accent="#ec4899" onClick={()=>setChatOpen(true)}/>
      <DiscordActionCard Icon={Dice5} title="Dice Roll" sub="Decide fast" accent="#eab308" onClick={rollGame}/>
      <button className="v6126ActionArrow" onClick={leaveMember}><ChevronRight size={20}/></button>
     </div></section>
    <SessionChatPanel chat={chat} chatInput={chatInput} setChatInput={setChatInput} sendChat={sendChat}/>
    <section className="v6126Channel"><div><Volume2 size={26}/><span><small>DISCORD BRIDGE</small><b>SquadUp Bot – Game Night</b><em>Polls, ready checks and VC prompts relay to your server</em></span></div><button><Share2 size={18}/></button><button><Users size={18}/>{members.length}</button><button><Settings size={18}/></button></section>
   </main>
...
function SquadDecisionPanel({votes,vote,rollGame,rolled}){
 const total=Math.max(1,Object.values(votes).reduce((a,b)=>a+b,0));
 return <section className="decisionPanel"><div className="decisionHead"><div><p className="accent">SQUAD DECISION ENGINE</p><h2>What are we playing?</h2><small>Vote first. Roll if democracy fails.</small></div><button onClick={rollGame}><Dice5 size={17}/> Roll now</button></div>
  <div className="decisionGrid">{decisionGames.map(g=><button key={g.id} onClick={()=>vote(g)}><img src={g.art}/><span><b>{g.title}</b><small>{votes[g.id]||0} votes · {gameDeal(g).owned} own</small><i><em style={{width:`${((votes[g.id]||0)/total)*100}%`}}></em></i></span></button>)}</div>
  {rolled&&<div className="rolledResult"><Dice5 size={18}/><b>Dice picked: {rolled.title}</b><small>Ready to send result to Discord.</small></div>}
 </section>
}
function SessionChatPanel({chat,chatInput,setChatInput,sendChat}){return <section className="sessionChat"><div className="decisionHead"><div><p className="accent">SESSION CHAT</p><h2>Lightweight squad chat</h2><small>For people you do not need to add to private Discord yet.</small></div></div>{chat.map((m,i)=><div className="sessionMsg" key={i}><img src={m.avatar}/><p><b>{m.name}</b>{m.text}</p></div>)}<div className="sessionInput"><input value={chatInput} onChange={e=>setChatInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')sendChat()}} placeholder="Quick coordination message..."/><button onClick={sendChat}><Send size={16}/></button></div></section>}
...
function Toast({text,clear}){useEffect(()=>{const t=setTimeout(clear,3500);return ()=>clearTimeout(t)},[text,clear]);return <div className="toast"><CheckCircle2/> {text}</div>}
