...
const users=['Shane','Leah','Tom','Maya','Sven','Jaron','Zoe','Liam','Kevin','Rook','Nyx','Valk'].map((name,i)=>({
 name, avatar:`https://api.dicebear.com/9.x/adventurer-neutral/svg?seed=${avatarSeeds[i]}&backgroundColor=0f0f14,1a122a,23183b&radius=50`,
...
function VoicePage(){
 ...
 const [selectedRoom,setSelectedRoom]=useState('Squad Alpha');
 const [input,setInput]=useState('Squad Mic (HyperX QuadCast)');
 const [output,setOutput]=useState('Squad Headset (Arctis 7)');
 const meters=[82,34,67,51,76,28,59,44];
 return <Page className="voiceExactPage"><MobileHeader/>
  <section className="voiceArenaExact">
   <header className="voiceExactHeader">
    <div>
      <p className="accent">LIVE VOICE ARENA</p>
      <h1>{selectedRoom}</h1>
      <p><span className="liveDot"></span>{joined?'Connected':'Previewing'} · 8 in voice · Low latency</p>
    </div>
    <div className="voiceHeaderActions">
      <button onClick={()=>setSelectedRoom(selectedRoom==='Squad Alpha'?'Night Raid':'Squad Alpha')}>Switch Room</button>
      <button><Settings size={17}/> Settings</button>
    </div>
   </header>

   <div className="voiceExactGrid">
    <aside className="voiceRoomsExact">
      <div className="panelTitleExact"><h2>Active Parties</h2><button>View All</button></div>
      {[
       ['Squad Alpha','HELLDIVERS™ 2','Looking for 1','4/5',games[1]],
       ['Night Raid','Elden Ring','Boss Run','3/4',games[0]],
       ['Rust Wipe Crew','RUST','Base Defense','6/8',games[3]],
       ['Chill Queue','Deep Rock Galactic','Casual VC','2/4',games[11] || games[2]]
      ].map((r,i)=><button key={r[0]} className={selectedRoom===r[0]?'voiceRoomCard active':'voiceRoomCard'} onClick={()=>setSelectedRoom(r[0])} style={{backgroundImage:`linear-gradient(90deg,rgba(8,10,18,.92),rgba(8,10,18,.55)),url(${r[4].art})`}}>
        <div><b>{r[0]}</b><span>{r[1]}</span><small>{r[2]}</small></div>
        <em>{r[3]}</em>
        <div className="roomStack">{users.slice(i,i+3).map(u=><img src={u.avatar}/>)}</div>
      </button>)}
    </aside>

    <main className="voiceStageExact">
      <div className="voiceStageGlow"></div>
      <div className="voiceCoreExact">
        <div className="pulseRing r1"></div>
        <div className="pulseRing r2"></div>
        <div className="pulseRing r3"></div>
        <Zap size={58}/>
        <span>{joined?'LIVE':'READY'}</span>
      </div>
      {users.slice(0,8).map((u,i)=><button className={`exactVoiceUser vu${i} ${i===1||i===4?'speaking':''}`} key={u.name}>
        <span className="voiceMeter" style={{height:`${meters[i]}%`}}></span>
        <img src={u.avatar}/>
        <b>{u.name}</b>
        <small>{['Leader','DPS','Support','Scout','Medic','Caller','Tank','Flex'][i]}</small>
        {i===0&&<Crown size={15}/>}
      </button>)}
    </main>

    <aside className="voiceControlsExact">
      <div className="panelTitleExact"><h2>Audio Test Center</h2><span className={testing?'status testing':'status'}>{testing?'Testing':'Ready'}</span></div>
      <label>Input Device<select value={input} onChange={e=>setInput(e.target.value)}><option>Squad Mic (HyperX QuadCast)</option><option>USB Microphone</option><option>Default System Mic</option></select></label>
      <div className="meterBox"><span style={{width:testing?'76%':'22%'}}></span><em>{testing?'Voice detected':'Idle'}</em></div>
      <label>Output Device<select value={output} onChange={e=>setOutput(e.target.value)}><option>Squad Headset (Arctis 7)</option><option>Speakers (Realtek)</option><option>Default Output</option></select></label>
      <button className="primary wide" onClick={()=>setTesting(!testing)}>{testing?'Stop Mic Test':'Start Mic Test'}</button>
      <button className="wide">Play Test Sound</button>

      <div className="voiceToggleList">
        <Toggle label="Noise Suppression" on={noise}/>
        <Toggle label="Echo Cancellation" on/>
        <Toggle label="Auto Gain Control" on/>
        <Toggle label="Spatial Audio"/>
      </div>

      <div className="voiceQuality">
        <div><b>12ms</b><small>Latency</small></div>
        <div><b>0.1%</b><small>Packet Loss</small></div>
        <div><b>48kHz</b><small>Quality</small></div>
      </div>
    </aside>
   </div>

   <footer className="voiceFooterExact">
    <button className={muted?'active danger':''} onClick={()=>setMuted(!muted)}><Mic size={18}/>{muted?'Unmute':'Mute'}</button>
    <button className={deafened?'active danger':''} onClick={()=>setDeafened(!deafened)}><Headphones size={18}/>{deafened?'Undeafen':'Deafen'}</button>
    <button onClick={()=>setNoise(!noise)}><SlidersHorizontal size={18}/>Noise</button>
    <button><Users size={18}/>Invite</button>
    <button><MessageCircle size={18}/>Open Chat</button>
    <button className={joined?'danger':'primary'} onClick={()=>setJoined(!joined)}>{joined?'Leave Voice':'Join Voice'}</button>
   </footer>
  </section>
 </Page>
}
...
function ProfilePage({wishlist,toggleWish,setSelected}){
 ...
 return <Page><MobileHeader/>
  <section className="profileHero">
   <img src={users[0].avatar}/><div><h1>Shane <Shield size={18}/></h1><p><span className="dot"></span>Online</p></div>
  ...
  <div className="profileGrid"><Panel title="About Me"><p>Squad leader. FPS enthusiast. Always up for a good time.</p><div className="titleChips"><span>LAN Captain</span><span>Night Stalker</span><span>Raid Caller</span></div></Panel><Panel title={`Wishlist (${wish.length})`}>{wish.map(g=><WishlistRow game={g} toggleWish={toggleWish} setSelected={setSelected}/>)}</Panel><Panel title="Recent Activity"><ActivityList/></Panel></div>
 </Page>
}
