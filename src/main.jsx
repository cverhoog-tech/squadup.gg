 ...
 return <Page className="voiceRenderPage"><MobileHeader/>
  <section className="voiceRenderShell">
    ...
    <section className="voiceRenderArena">
      ...
      <button className="arenaSideArrow left"><ChevronLeft size={28}/></button>
      <button className="arenaSideArrow right"><ChevronRight size={28}/></button>
    ...
    <section className="activeSquadsRender">
      ...
      <button className="squadArrow left"><ChevronLeft size={24}/></button>
      <button className="squadArrow right"><ChevronRight size={24}/></button>
      <div className="squadCardsRender">
        {squadCards.map((s,i)=><button className={`renderSquadCard ${s.selected?'selected':''}`} key={s.name} onClick={()=>setActiveSquad(s.name)} style={{backgroundImage:`linear-gradient(180deg,rgba(8,9,18,.08),rgba(8,9,18,.93)),url(${s.art})`}}>
    ...
    <section className="voiceActivityRender">
      ...
      <div className="activityAvatarRow">
        {users.slice(1,9).map((u,i)=><button className={`activityAvatar ${(i===5||i===6)?'red':i===2||i===7?'purple':'green'}`} key={u.name}>
          <img src={u.avatar}/>
          <Mic size={16}/>
          <strong>{u.name}</strong>
        </button>)}
