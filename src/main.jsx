...
function GameDetail({game,onClose,wishlist,toggleWish}){const liked=wishlist.includes(game.id);return <div className="modal"><div className="storefront"><button className="close" onClick={onClose}>×</button><div className="storeHero" style={{backgroundImage:`linear-gradient(90deg,#070610 35%,rgba(6,5,12,.2)),url(${game.hero})`}}><h1>{game.title}</h1><p>{game.desc}</p><a className="steam" href={game.steam} target="_blank">Play on Steam</a><button className={liked?'heart liked':'heart'} onClick={()=>toggleWish(game)}>♥ Wishlist</button><button className="primary">Recommend Friend</button><button>Ask Squad to Buy</button></div><div className="storeTabs"><button>Overview</button><button>Activity</button><button>Media</button><button>Achievements</button></div><Section title="Screenshots & similar" games={games.slice(1,7)} /></div></div>}
