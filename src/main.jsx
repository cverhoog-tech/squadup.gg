...
function ImmersiveExplore({games,wishlist,toggleWish,setSelected}){
 ...
 const sections=[
  {type:'double', a:'Continue Playing', b:'Trending With Friends', start:0},
  {type:'feature', title:'SquadUp Spotlight', flip:false, start:2},
  {type:'double', a:'Recommended For You', b:'Popular In Your Squad', start:4},
  {type:'feature', title:'Co-op Nights', flip:true, start:6},
  {type:'double', a:'Survival Worlds', b:'New & Upcoming', start:1},
  {type:'feature', title:'Weekend LAN Picks', flip:false, start:3},
  {type:'double', a:'Discounts Worth Watching', b:'Hidden Gems', start:5},
  {type:'feature', title:'PvP Hotlist', flip:true, start:7},
 ];
 return <div className="immersiveExploreV51">
   <section className="megaExploreHero" style={{backgroundImage:`linear-gradient(90deg,#070912 10%,rgba(7,9,18,.25) 58%,#070912 100%),url(${hero.hero})`}}>
    <button className="heroRound left"><ChevronLeft size={22}/></button>
    <div className="megaHeroCopy">
      <p className="accent">Featured</p>
      <h1>{hero.title}</h1>
      <h2>{hero.sub}</h2>
      <p>{hero.desc}</p>
      <div className="megaHeroActions">
        <button className="primary" onClick={()=>setSelected(hero)}>View Game</button>
        <a className="steamBtn" href={hero.steam} target="_blank"><Steam size={15}/> Open on Steam</a>
      </div>
    </div>
    <aside className="heroMiniStack desktopOnly">
      {games.slice(1,5).map(g=><button key={g.id} onClick={()=>setSelected(g)} style={{backgroundImage:`linear-gradient(90deg,rgba(0,0,0,.2),rgba(0,0,0,.42)),url(${g.art})`}}><span>{g.title}</span></button>)}
    </aside>
    <button className="heroRound right"><ChevronRight size={22}/></button>
    <div className="heroDots"><span className="active"></span><span></span><span></span><span></span><span></span></div>
   </section>

   <div className="immersiveFeed">
    {sections.map((s,i)=> s.type==='double'
      ? <section className="doubleCarouselBlock" key={i}>
          <Carousel title={s.a} items={games.concat(games).slice(s.start,s.start+10)} wishlist={wishlist} toggleWish={toggleWish} setSelected={setSelected}/>
          <Carousel title={s.b} items={games.concat(games).slice(s.start+3,s.start+13)} wishlist={wishlist} toggleWish={toggleWish} setSelected={setSelected}/>
        </section>
      : <FeatureListBlock key={i} title={s.title} flip={s.flip} items={games.concat(games).slice(s.start,s.start+6)} wishlist={wishlist} toggleWish={toggleWish} setSelected={setSelected}/>
    )}
   </div>
  </div>
}
...
function VoicePage(){
 return <Page><MobileHeader/>
  <section className="voicePanel">
   <div className="partyHead"><div><h1>Squad Alpha</h1><p><span className="dot"></span>8 In Voice</p></div><button><Settings size={17}/>Settings</button></div>
   <div className="voiceOrbit">{users.slice(0,7).map((u,i)=><button className={`voiceOrb v${i}`}><img src={u.avatar}/><span>{u.name}</span>{i===0&&<Crown size={15}/>}</button>)}<div className="crest"><Zap size={56}/></div></div>
   <aside className="voiceSettings desktopOnly"><h3>Voice Settings</h3><label>Input Device<select><option>Squad Mic (HyperX)</option></select></label><input type="range" defaultValue="70"/><label>Output Device<select><option>Squad Headset</option></select></label><input type="range" defaultValue="84"/><Toggle label="Noise Suppression" on/><Toggle label="Voice Activity" on/><Toggle label="3D Spatial Audio"/></aside>
   <div className="voiceButtons"><button><Mic/>Mute</button><button><Headphones/>Deafen</button><button><Settings/>Settings</button><button className="danger">Leave Voice</button><button><Users/>Invite</button><button>Raise Hand</button></div>
  </section>
 </Page>
}
...
function GameCard({game,wishlist=[],toggleWish=()=>{},setSelected}){
 ...
 return <article className="gameCard" onClick={()=>setSelected(game)}><img src={game.art}/><div><b>{game.title}</b><small>{game.playing} playing</small><em>{game.score}</em></div><button className={liked?'like liked':'like'} onClick={(e)=>{e.stopPropagation();toggleWish(game.id)}}><Heart size={15} fill={liked?'currentColor':'none'}/></button></article>
}
