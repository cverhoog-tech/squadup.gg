...
import {
  ...
  ChevronLeft, X, Share2, Bookmark, Play, Steam, Send, Smile, Gift, Dice5,
...
function ImmersiveExplore({games,wishlist,toggleWish,setSelected}){
 const hero=games[0]||window.games?.[0];
 const feed=['Popular Right Now','Recommended for You','Trending Now','Co-op Picks','Survival Squad Nights','Discounts','New & Upcoming','Your Squad Might Like'];
 return <div>
   <section className="exploreHero" style={{backgroundImage:`linear-gradient(90deg,#070912 12%,rgba(7,9,18,.30)),url(${hero.hero})`}}>
    <div><p className="accent">FEATURED</p><h1>{hero.title}</h1><p>{hero.desc}</p><button className="primary" onClick={()=>setSelected(hero)}>View Game</button><a className="steamBtn" target="_blank" href={hero.steam}><Steam size={15}/>Open on Steam</a><div className="tags">{hero.tags.map(t=><span>{t}</span>)}</div></div><ChevronRight className="heroArrow"/>
   </section>
   {feed.map((title,i)=><Carousel key={title} title={title} items={games.concat(games).slice(i,i+9)} wishlist={wishlist} toggleWish={toggleWish} setSelected={setSelected}/>)}
 </div>
}
