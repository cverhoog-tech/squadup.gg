...
function ImmersiveExplore({games,wishlist,toggleWish,setSelected}){
 ...
 return <div className="exploreExactV52">
   <section className="exactHero" style={{backgroundImage:`linear-gradient(90deg,#070912 7%,rgba(7,9,18,.35) 60%,#070912 100%),url(${hero.hero})`}}>
   ...
   <ExactCarousel title="Play Your Way" subtitle="Top picks across all genres" items={games.concat(games).slice(0,9)} wishlist={wishlist} toggleWish={toggleWish} setSelected={setSelected}/>

   <section className="exactFeatureSplit">
     <div className="exactSpotlight" style={{backgroundImage:`linear-gradient(90deg,#100d1b 10%,rgba(16,13,27,.42)),url(${games[4]?.hero || games[4]?.art})`}}>
       <p className="accent">Live Events</p>
       <h2>Squad Battles</h2>
       <p>Compete. Climb. Conquer. Join events, complete challenges and earn exclusive rewards.</p>
       <button className="primary">View Events</button>
     </div>
     <div className="exactRankList">
       <div className="listHeader"><h2>Trending Now</h2><small>See what’s blowing up</small></div>
       {games.slice(1,6).map((g,i)=><button className="rankRow" key={g.id} onClick={()=>setSelected(g)}>
         <b>{i+1}</b><img src={g.art}/><span>{g.title}</span><small>{g.playing} playing</small><em>{g.score}</em>
       </button>)}
     </div>
   </section>

   <ExactCarousel title="Popular Categories" subtitle="Jump into what you love" compact items={games.concat(games).slice(5,14)} wishlist={wishlist} toggleWish={toggleWish} setSelected={setSelected}/>

   <section className="exactBottomSplit">
     <div className="exactUpcoming">
      <h2>New & Upcoming</h2><p>Games you don’t want to miss</p>
      {['Black Myth: Wukong','Star Wars Outlaws','Warhammer 40,000: Space Marine 2','Assassin’s Creed Shadows'].map((name,i)=><button className="upcomingRow" key={name}>
        <img src={games[(i+7)%games.length].art}/><span>{name}</span><small>{['AUG 20, 2024','AUG 30, 2024','SEP 9, 2024','OCT 15, 2024'][i]}</small><em>♡ Wishlist</em>
      </button>)}
     </div>
     <div className="exactCommunity" style={{backgroundImage:`linear-gradient(90deg,#120d22 15%,rgba(18,13,34,.32)),url(${games[1].hero})`}}>
  ...
  <div className="exactCarouselViewport">
   <div className="exactCardTrack" ref={ref}>
    {items.map((g,i)=><button className="exactGameTile" key={g.id+i} onClick={()=>setSelected(g)}>
      ...
      <div><b>{g.title}</b><small>{g.sub}</small></div>
      <em>{g.score}</em>
    </button>)}
   </div>
  </div>
